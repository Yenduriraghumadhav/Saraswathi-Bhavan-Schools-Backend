...
router.post("/Thirdclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
        ...
        if (mainStudent.stdclass !== 3) {
            return res.status(400).json({
                error: `Class mismatch: This student belongs to Class ${mainStudent.stdclass}, but you are trying to add marks for Class 3.`
            });
        }
...
router.put("/Thirdclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
        ...
        const student = await thirdgrademodel.findOne({ stdRollNumber: rollnumber });
        if (!student) {
...
router.delete("/Thirdclassstudents", requireRole(["admin"]), async (req, res) => {
    try {
        ...
        const deleted = await thirdgrademodel.findOneAndDelete({ stdRollNumber: rollnumber });
        if (!deleted) {
            return res.status(404).json({ message: "Student not exists on that rollnumber" });
        }
