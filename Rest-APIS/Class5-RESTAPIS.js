...
const fifthgrademodel = require("../Schema-details/Class5-schema");
...
router.post("/fifthclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
        ...
        if (mainStudent.stdclass !== 5) {
            return res.status(400).json({
                error: `Class mismatch: This student belongs to Class ${mainStudent.stdclass}, but you are trying to add marks for 5th Class.`
            });
        }

        let FifthGradestudent = await fifthgrademodel.findOne({ stdRollNumber });

        if (!FifthGradestudent) {
            FifthGradestudent = new fifthgrademodel({
        ...
        const existingResult = FifthGradestudent.result?.[resultType];
        ...
        FifthGradestudent.result[resultType] = result;
        FifthGradestudent.markModified("result");
        const saved = await FifthGradestudent.save();
...
router.put("/fifthclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
        ...
        const student = await fifthgrademodel.findOne({ stdRollNumber: rollnumber });
        if (!student) {
...
router.delete("/fifthclassstudents", requireRole(["admin"]), async (req, res) => {
    try {
        const { stdRollNumber: rollnumber } = req.body;
        if (!rollnumber) {
            return res.status(400).json({ error: "rollnumber is required" });
        }
        const deleted = await fifthgrademodel.findOneAndDelete({ stdRollNumber: rollnumber });
        if (!deleted) {
            return res.status(404).json({ message: "Student not exists on that rollnumber" });
        }
...
router.get("/fifthclassstudents", requireRole(["student", "teacher", "admin"]), async (req, res) => {
    try {
        const students = await fifthgrademodel.find();
