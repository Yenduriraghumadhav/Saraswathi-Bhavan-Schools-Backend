...
const ninthclassstudents = require("../Schema-details/Class9-schema");
...
router.post("/ninthclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
        ...
        if (mainStudent.stdclass !== 9) {
            return res.status(400).json({
                error: `Class mismatch: This student belongs to Class ${mainStudent.stdclass}, but you are trying to add marks for 9th Class.`
            });
        }

        let ninthGradeStudent = await ninthclassstudents.findOne({ stdRollNumber });

        if (!ninthGradeStudent) {
            ninthGradeStudent = new ninthclassstudents({
        ...
        const existingResult = ninthGradeStudent.result?.[resultType];
        ...
        ninthGradeStudent.result[resultType] = result;
        ninthGradeStudent.markModified("result");
        const saved = await ninthGradeStudent.save();
...
router.put("/ninthclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
        ...
        const student = await ninthclassstudents.findOne({ stdRollNumber: rollnumber });
        if (!student) {
...
router.delete("/ninthclassstudents", requireRole(["admin"]), async (req, res) => {
    try {
        const { stdRollNumber: rollnumber } = req.body;
        if (!rollnumber) {
            return res.status(400).json({ error: "rollnumber is required" });
        }
        const deleted = await ninthclassstudents.findOneAndDelete({ stdRollNumber: rollnumber });
        if (!deleted) {
            return res.status(404).json({ message: "Student not exists on that rollnumber" });
        }
...
router.get("/ninthclassstudents", requireRole(["student", "teacher", "admin"]), async (req, res) => {
    try {
        const students = await ninthclassstudents.find();
