...
router.post("/sixthclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
        ...
        if (mainStudent.stdclass !== 6) {
            return res.status(400).json({
                error: `Class mismatch: This student belongs to Class ${mainStudent.stdclass}, but you are trying to add marks for 6th Class.`
            });
        }
...
router.put("/sixthclassstudents", requireRole(["teacher", "admin"]),TeacherCLassVerification, async (req, res) => {
    try {
